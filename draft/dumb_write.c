...
int main()
{
	...
	uint64_t start_zo[row+1] = { 0 , 3, 6 , 8 ,10 };
	/* uint32_t colid_zo[nnz] = { 0, 1, 7, 0, 4, 6, 1, 4, 2, 5 }; */
	uint32_t colid_zo[nnz] = { 0 , 2 , 7 & mask, 0 & mask, 4 & mask, 6 & mask, 1 &mask, 4 & mask, 2 & mask, 5& mask};
	uint32_t colid_size = 10 ;
	uint32_t map_zo_pol[row] = { 0, 0, 1, 1 };
	const uint32_t pol_nb = 2 ;
	uint32_t pol_start [pol_nb+1] = { 0 , 3 , 5 } ;
	const uint32_t pol_nnz = pol_start[pol_nb] ;
	uint32_t pol_data [pol_nnz] = { 2, 3, 4, 9, 8 };
